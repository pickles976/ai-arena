...
import { Asteroid, Base, Bullet, EnergyCell, Obstacle, Ship } from '../objects/objects.js';
import { Vector2D } from '../engine/physics.js';
import { create_UUID, randomInRange } from '../util/utils.js';
    ...
    indexObjects(gameObjectList: Array<GameObject>) {
        ...
        for (let i = 0; i < gameObjectList.length; i++) {
            ...
            if (gameObj != undefined && gameObj != null) {
                ...
                switch (gameObj?.type) {
                    case 'ASTEROID':
                        if (gameObj instanceof Asteroid) this.asteroids.push(gameObj as Asteroid);
                        break;
                    case 'OBSTACLE':
                        if (gameObj instanceof Obstacle) this.obstacles.push(gameObj as Obstacle);
                        break;
                    case 'ENERGY_CELL':
                        if (gameObj instanceof EnergyCell) this.energyCells.push(gameObj as EnergyCell);
                        break;
                    case 'SHIP':
                        if (gameObj instanceof Ship) this.ships.push(gameObj as Ship);
                        break;
                    case 'BULLET':
                        if (gameObj instanceof Bullet) this.bullets.push(gameObj as Bullet);
                        break;
                    case 'BASE':
                        if (gameObj instanceof Base) this.bases.push(gameObj as Base);
                        break;
                    default:
                        break;
                }
